...
class FileUploadController {
    ...
    public ResponseEntity<String> handleImport( @RequestParam String importId, @RequestParam String title, @RequestParam String filename) {
        return restTemplate.execute("http://upload-service:3333/upload/" + importId, HttpMethod.GET, null,
                new ResponseExtractor<ResponseEntity<String>>() {
                    ...
                    public ResponseEntity<String> extractData(ClientHttpResponse response) throws IOException {
                        try {
                            saveFile(response.getBody(), uploadDir + File.separator + importId, filename);
                            handleUpload(title, filename);
    ...
    private ResponseEntity<String> handleUpload(String title, String filename) {
